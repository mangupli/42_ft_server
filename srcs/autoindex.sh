...
if grep "autoindex on;" /etc/nginx/sites-enabled/localhost > /dev/null; then
  ...
  sed -i 's/root \/var\/www\//root \/var\/www\/wordpress\//' /etc/nginx/sites-available/localhost
else
  echo "autoindex is off, turning it on"
  sed -i 's/autoindex off/autoindex on/' /etc/nginx/sites-available/localhost
  sed -i 's/root \/var\/www\/wordpress\//root \/var\/www\//' /etc/nginx/sites-available/localhost
fi
nginx -s reload
